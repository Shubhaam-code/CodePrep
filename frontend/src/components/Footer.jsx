    ...
    <footer className="border-t border-white/5 bg-[#080810]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          ...
          <div className="col-span-2 lg:col-span-1">
            ...
            <div className="flex gap-3">
              {[GithubIcon, TwitterIcon, LinkedinIcon].map((Icon, i) => (
                <button key={i} className="w-8 h-8 rounded-lg bg-white/5 border border-white/8 flex items-center justify-center text-gray-500 hover:text-white hover:border-[#FF7A00]/40 transition-all">
                  <Icon size={14} />
                </button>
              ))}
            </div>
          ...
          {Object.entries(links).map(([title, items]) => (
            <div key={title}>
              ...
              <ul className="space-y-2.5">
                {items.map(item => (
                  <li key={item}>
                    <a href="#" className="text-gray-500 hover:text-gray-300 text-xs transition-colors">{item}</a>
                  </li>
                ))}
              </ul>
            </div>
        ...
        <div className="border-t border-white/5 pt-6 flex flex-col sm:flex-row items-center justify-between gap-3">
          <p className="text-gray-600 text-xs">© {new Date().getFullYear()} CodePrep AI. All rights reserved.</p>
          <div className="flex items-center gap-1 text-xs text-gray-600">
            <span>Built with</span><span className="text-[#FF7A00]">♥</span><span>for engineers who want to get hired</span>
          </div>
        </div>
      </div>
    </footer>
